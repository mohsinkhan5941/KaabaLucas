...
<div x-data="{
        ...
        init() {
            // Listen for dynamic options changes (only affects comboboxes that use this feature)
            this.$el.addEventListener('options-changed', (event) => {
                this.allOptions = [...event.detail.options];
                this.options = [...event.detail.options];
                // Always update filtered options for searchable comboboxes
                if (this.searchable) {
                    this.getFilteredOptions(this.searchQuery || '');
                }
            });
    ...
    <div class="relative">
        ...
        x-ref="searchField"
        x-init="$watch('isOpen', value => { if (value && $refs.searchField) $refs.searchField.focus(); })"
/>
